...
The `step` option on a table applies `fragment` to the whole `<table>`, revealing every row at once.
Use the `rowstep` option instead to apply `fragment` to each body row individually, revealing the table one row at a time:
...
`rowstep` only affects body rows — the header row (if any) is always shown.
