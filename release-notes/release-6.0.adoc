= Asciidoctor reveal.js 6.0
:source-highlighter: highlight.js
:highlightjs-theme: a11y-dark.css
:highlightjs-languages: asciidoc
:icons: font
:imagesdir: images/
// reveal.js config
:customcss: release-6.0.css
:revealjs_theme: blood
:revealjs_hash: true
:revealjs_width: 1080

== New Features icon:rocket[set=fas]

Powered by https://revealjs.com[reveal.js] 6.0

== Image lightbox

Click the picture to open it in a full-screen lightbox icon:search-plus[set=fas]

image::cute-cat-1.jpg[A cute cat,height="320px",data-preview-image="images/cute-cat-1.jpg"]

== Website preview

Open a link in a full-screen iframe overlay icon:search-plus[set=fas]

https://docs.asciidoctor.org/[Open the Asciidoctor docs,preview=true]

== Image as a website preview

Click the image to preview a website icon:hand-pointer[set=far]

image::asciidoctor-logo.svg[AsciiDoc website,height="220px",data-preview-link="https://asciidoc.org/"]

== Collapsible blocks

Click to reveal hidden content icon:hand-pointer[set=far]

[%collapsible]
.Click to expand
====
Content hidden behind a toggle, rendered as a native `<details>`/`<summary>` widget.
====

== Reveal a table row by row

Use `rowstep` to apply `fragment` to each body row individually

[%rowstep,cols="1,1",options=header]
|===
|Name |Category

|Firefox |Browser
|Chrome |Browser
|===

[transition=fade,transition-speed=slow]
== Learn More!

* https://github.com/asciidoctor/asciidoctor-reveal.js/[Asciidoctor reveal.js]
* https://revealjs.com[reveal.js]
* https://github.com/asciidoctor/asciidoctor/[Asciidoctor]
* https://asciidoc.org/[What is AsciiDoc?]
* https://github.com/asciidoctor/asciidoctor-reveal.js/raw/main/examples/release-6.0.adoc[Source of this presentation (AsciiDoc)]
