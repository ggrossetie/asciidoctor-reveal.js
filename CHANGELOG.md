...
### Enhancements
  ...
  * Update Asciidoctor to 4.0 (stable) and drop the workarounds that are no longer needed; the peer dependency range is now `>=4.0.0 <5.0.0`, with no more alpha caveat
  ...
  * Support collapsible blocks (`[%collapsible]` on an example block, Ruby and JS converters); this rendered as a regular `.exampleblock` `<div>` instead of Asciidoctor's own html5 converter's `<details>`/`<summary>`, since this converter always used its own example block rendering regardless of the `collapsible`/`open` options — also ships the `asciidoctor-default.css` `<details>`/`<summary>` styling, since reveal.js has none of its own (#484)

### Bug Fixes

  * Stop wrapping an open block's content in a `.content` div when it has one of reveal.js's `r-stack`/`r-hstack`/`r-vstack` layout roles (Ruby and JS converters); those utilities style their *direct* children (e.g. `.r-stack > *` for stacking elements on top of each other), so the `.content` wrapper put everything one level too deep for reveal.js's CSS to reach, leaving e.g. `[.r-stack]`-wrapped images stacked vertically like normal content instead of on top of each other (#520)
  * Style block quote/verse attribution (right/left-aligned, distinct italic size) the same way `asciidoctor-default.css` does; reveal.js's own core CSS sets `.reveal .slides { text-align: center }`, which otherwise centered the attribution and left it visually identical to the quoted text, since this converter didn't ship any `.quoteblock`/`.verseblock` CSS of its own (#506)
  * Scope the `highlightjs`/`highlight.js` syntax highlighter override to the `revealjs`/`reveal.js` backend (Ruby and JS converters); registering it replaces the built-in highlight.js syntax highlighter process-wide (the `SyntaxHighlighter` registry has no notion of "per backend"), so simply requiring this converter used to change how *any* backend rendered `:source-highlighter: highlightjs` source blocks — e.g. adding `data-noescape`/`data-line-numbers` and the reveal.js highlight plugin docinfo to plain `html5` output. Delegates to Asciidoctor's own built-in adapter whenever the backend isn't revealjs, so requiring this converter alongside others is now a no-op for them (#489)
  * Omit titleless slides (`== !`) from the table of contents (`toc::[]`/`:toc:`), Ruby and JS converters; `convert_outline` had no notion of the `!`-means-titleless convention `convert_section` already uses to hide a slide's `<h2>`, so a titleless slide showed up in the ToC as a literal, unclickable-looking "!" entry (#236)
  * Support `:title-slide-background-gradient:` (Ruby and JS converters); every other `title-slide-background-*` attribute (`color`, `image`, `video`, `size`, `repeat`, `position`, `opacity`, `iframe`, `transition`) was already carried over to the title slide's `data-background-*` attributes, but `gradient` was missing from that list, even though the equivalent `background-gradient` attribute already works on regular sections (#514)
  * Wrap the document title in a proper title-slide `<section>` for embedded conversion (`--no-header-footer` with `showtitle`), instead of a bare `<h1>` rendered outside any slide (Ruby and JS converters); the stray `<h1>` broke the reveal.js slide model when the embedded fragment was included as-is on a page (e.g. via a static site generator), and had no reason to differ from the standalone title slide (#270)
  * Fix syntax colouring getting corrupted (duplicated, nested `<span>`s producing washed-out/uncoloured code) whenever `[source,...,highlight=...]` or `%linenums` triggers reveal.js's line-numbers plugin, with `:source-highlighter: highlightjs` (Ruby and JS converters); the docinfo footer script called `hljs.configure({ ignoreUnescapedHTML: true })` *after* registering the reveal.js `highlight` plugin, but that plugin highlights every code block as soon as it registers itself (`Reveal.initialize()` is already running by the time this footer script executes), so every block got highlighted once with the wrong hljs config before the correct config was ever applied; moved `hljs.configure()` before the plugin registers, and dropped the redundant `hljs.highlightAll()` call the plugin's own `highlightOnLoad` already made unnecessary (#525)
  * Set the `muted` attribute on local `video::` blocks with the `muted` option (Ruby and JS converters); `options="autoplay,muted"` autoplayed but never actually muted the `<video>` element, since `muted` was the only playback option (`autoplay`, `controls`, `loop`) not carried over to the generated tag — matching Asciidoctor's own html5 converter, which already sets it (#523)
  ...
  * Align the `<summary>` disclosure triangle with its text in collapsible blocks (`[%collapsible]`); `display: block` stacked the marker above the summary text instead of beside it, since `<summary>`'s default `display: list-item` (which lays the marker inline with the content) was overridden without replacing that behavior — now `display: flex` with `padding-left`
...
### Build
  ...
  * Remove the `// .<case-name>`, `// :include:` and `// :header_footer:` comment lines from `test/fixtures/standalone/*.adoc`; these were asciidoctor-doctest directives (case name, xpath selector, standalone-conversion flag) left over from before the migration to asciidoc-testkit, which now gets the same information from each fixture's `.config.json` sidecar and isn't aware of these comments at all
  ...
  * Wire `testkit:test:js` into CI, on the primary Linux/CRuby leg only (it exercises OS-independent Node.js code, so one platform is enough); the 2 permanent, documented gaps (`source-prettify`/`source-rouge`, third-party Ruby-only syntax highlighters never ported to the JS converter) are excluded via asciidoc-testkit's own `--ignore <family/name>:<reason>` flag instead of `continue-on-error: true`, so the job now fails on any other (real) regression instead of swallowing all of them
